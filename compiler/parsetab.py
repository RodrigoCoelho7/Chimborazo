...
_lr_productions = [
  ...
  ('declV -> VAR ids : tipo ENTER','declV',5,'p_declV','yacc.py',38),
  ('ids -> ID','ids',1,'p_ids_1','yacc.py',43),
  ('ids -> ids , ID','ids',3,'p_ids_mult','yacc.py',47),
  ('tipo -> ENTERO','tipo',1,'p_tipo_int','yacc.py',51),
  ('tipo -> REAL','tipo',1,'p_tipo_REAL','yacc.py',55),
  ('tipo -> BOOLEANO','tipo',1,'p_tipo_BOOL','yacc.py',59),
  ('declL -> LISTA ID = lista ENTER','declL',5,'p_declL','yacc.py',67),
  ('lista -> [ ]','lista',2,'p_lista_vazia','yacc.py',70),
  ('lista -> [ elementos ]','lista',3,'p_lista_elem','yacc.py',73),
  ('elementos -> elemento','elementos',1,'p_elementos_1','yacc.py',76),
  ('elementos -> elementos , elemento','elementos',3,'p_elementos_mult','yacc.py',79),
  ('elemento -> INT','elemento',1,'p_elemento_INT','yacc.py',82),
  ('elemento -> FLOAT','elemento',1,'p_elemento_FLOAT','yacc.py',85),
  ('elemento -> BOOLEANO','elemento',1,'p_elemento_BOOL','yacc.py',88),
  ('elemento -> STRING','elemento',1,'p_elemento_STRING','yacc.py',91),
  ('elemento -> ID','elemento',1,'p_elemento_ID','yacc.py',94),
  ('elemento -> lista','elemento',1,'p_elemento_lista','yacc.py',97),
  ('declF -> FUNCION ID ( parametros ) : ENTERO ENTER declarationsF statementsI TAB DEVUELVE return','declF',13,'p_declF_parametros','yacc.py',104),
  ('declF -> FUNCION ID ( ) : ENTERO ENTER declarationsF statementsI TAB DEVUELVE return','declF',12,'p_declF_vazia','yacc.py',108),
  ...
  ('declarationsF -> TAB decl','declarationsF',2,'p_declarationsF_1','yacc.py',122),
  ('declarationsF -> declarationsF TAB decl','declarationsF',3,'p_declarationsF_mult','yacc.py',126),
  ('return -> INT ENTER','return',2,'p_return_INT','yacc.py',130),
  ('return -> ID ENTER','return',2,'p_return_ID','yacc.py',133),
  ('statementsI -> TAB stat','statementsI',2,'p_statementsI_1','yacc.py',140),
  ('statementsI -> statementsI TAB stat','statementsI',3,'p_statementsI_mult','yacc.py',145),
  ('statements -> stat','statements',1,'p_statements_1','yacc.py',150),
  ('statements -> statements stat','statements',2,'p_statements_mult','yacc.py',153),
  ('stat -> atrib ENTER','stat',2,'p_stat_atrib','yacc.py',156),
  ('stat -> conditions','stat',1,'p_stat_conditions','yacc.py',160),
  ('stat -> ciclos','stat',1,'p_stat_ciclos','yacc.py',164),
  ('atrib -> ID = exp','atrib',3,'p_atrib','yacc.py',169),
  ('exp -> exp + termo','exp',3,'p_exp_soma','yacc.py',173),
  ('exp -> exp - termo','exp',3,'p_exp_sub','yacc.py',177),
  ('exp -> termo','exp',1,'p_exp_termo','yacc.py',181),
  ('termo -> termo * fator','termo',3,'p_termo_mul','yacc.py',185),
  ('termo -> termo / fator','termo',3,'p_termo_div','yacc.py',189),
  ('termo -> termo ^ fator','termo',3,'p_termo_pot','yacc.py',193),
  ('termo -> fator','termo',1,'p_termo_fator','yacc.py',197),
  ('fator -> INT','fator',1,'p_fator_INT','yacc.py',201),
  ('fator -> FLOAT','fator',1,'p_fator_FLOAT','yacc.py',205),
  ('fator -> ID','fator',1,'p_fator_ID','yacc.py',209),
  ('fator -> ( exp )','fator',3,'p_fator_exp','yacc.py',213),
  ('conditions -> SI expL ENTONCES ENTER statementsI','conditions',5,'p_conditions_si','yacc.py',221),
  ('conditions -> SI expL ENTONCES ENTER statementsI CASO CONTRARIO ENTER statementsI','conditions',9,'p_conditions_si_no','yacc.py',226),
  ('expL -> termoB','expL',1,'p_expL_1','yacc.py',229),
  ('expL -> expL OR termoB','expL',3,'p_expL_mult','yacc.py',232),
  ('termoB -> fatorB','termoB',1,'p_termoB_1','yacc.py',235),
  ('termoB -> termoB AND fatorB','termoB',3,'p_termoB_mult','yacc.py',238),
  ('fatorB -> condition','fatorB',1,'p_fatorB_condition','yacc.py',241),
  ('fatorB -> BOOLEANO','fatorB',1,'p_fatorB_BOOL','yacc.py',244),
  ('fatorB -> ( expL )','fatorB',3,'p_fatorB_expL','yacc.py',247),
  ('condition -> exp op exp','condition',3,'p_condition','yacc.py',250),
  ('op -> >','op',1,'p_op_maior','yacc.py',253),
  ('op -> <','op',1,'p_op_menor','yacc.py',256),
  ('op -> IGUAL','op',1,'p_op_IGUAL','yacc.py',259),
  ('op -> DIFERENTE','op',1,'p_op_DIFERENTE','yacc.py',262),
  ('op -> > IGUAL','op',2,'p_op_maiorIGUAL','yacc.py',265),
  ('op -> < IGUAL','op',2,'p_op_menorIGUAL','yacc.py',268),
  ('ciclos -> ENCUANTO expL HACER ENTER statementsI','ciclos',5,'p_ciclos_while','yacc.py',275),
  ('ciclos -> PARA expL SIGUIENTE atrib . ENTER','ciclos',6,'p_ciclos_for_1','yacc.py',278),
  ('ciclos -> PARA expL SIGUIENTE atrib HACER ENTER statementsI','ciclos',7,'p_ciclos_for_mult','yacc.py',281),
]
